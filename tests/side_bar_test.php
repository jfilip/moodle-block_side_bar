...
class blockSideBarTestcase extends advanced_testcase {
    ...
    private function validate_sidebar_course_section($section, $sectionnum, $courseid) {
        ...
        $this->assertEquals(get_string('sectionsummary', 'block_side_bar', (string)html_writer::link($reseturl, $reseturl)),
                $section->summary);
    ...
    private function create_sidebar_course_section($courseid, $sectionnum) {
        ...
        $options = array('course' => $courseid, 'section' => $sectionnum);
        $section = $DB->get_record('course_sections', $options, 'id, section, name, visible');
    ...
    /**
     * Test that the Side Bar block activity section is appropriately added to a course when that course contains orphaned
     * sections but no activity modules within an orphaned section (orphaned being sections that exist beyond the number of
     * sections configured for the course).
     */
    ...
    /**
     * Test that the Side Bar block activity section is appropriately added to a course when that course contains one orphaned
     * section and that section contains an activity module (orphaned being sections that exist beyond the number of sections
    ...
    /**
     * Test that the Side Bar block activity section is appropriately added to a course when that course contains multiple
     * orphaned sections but only the "highest" orphaned section contains an activity module (orphaned being sections that
     * exist beyond the number of sections configured for the course).
     */
    ...
    /**
     * Test that the Side Bar block activity section is appropriately added to a course when that course contains multiple
     * orphaned sections but only the "lowest" orphaned section contains an activity module (orphaned being sections that
     * exist beyond the number of sections configured for the course).
     */
    ...
    /**
     * Test that the old sidebar course section migrates into the new position when there are no "fillter" sections that have
     * been created.
     */
