...
function block_side_bar_create_section($course) {
    ...
    $options = array('course' => $course->id, 'section' => $sectionnum);
...
function block_side_bar_migrate_old_section($course, $sectionnum) {
    ...
    foreach ($orphanedsections as $orphanedsection) {
        ...
        if (!empty($orphanedsection->sequence)) {
            ...
            $summarymatch = get_string('sectionsummary', 'block_side_bar', (string)html_writer::link($reseturl, $reseturl)) ==
                    $orphanedsection->summary;
    ...
    $options = array('course' => $course->id, 'section' => $sectionend);
    $sectioninfo = $DB->get_record('course_sections', $options, 'id, section, name, visible');
...
function block_side_bar_move_section($course, $sectionnum) {
    ...
    $options = array('course' => $course->id, 'section' => $newsection);
    $sectioninfo = $DB->get_record('course_sections', $options, 'id, section, name, visible');
    if (false === $sectioninfo) {
        debugging('could not find section '.$sectionend, DEBUG_DEVELOPER);
        return null;
